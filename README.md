...
```
# ビルドツール
sudo apt-get install git autoconf libtool automake build-essential gettext cmake python3 curl pkg-config libglib2.0-dev
...
- 入力候補フォームを別ウィンドウではなくメインウィンドウの中に表示
- 入力候補フォームが出た瞬間消えてぬるぽするバグをアドホックに修正
- 入力候補フォームをクリックすると消えてぬるぽするバグをアドホックに修正
